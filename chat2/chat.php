...
//connect to database
$db = mysqli_connect('mariadb', 'cs431s36', 'gaeFa6xu', 'cs431s36');
...
try { 
    ...
    switch($action) {
        case 'poll':
           ...
           $stmt->execute();
           $stmt->bind_result($id, $name, $message, $session_id, $date_created);
        ...
        case 'send':
            ...
            $query = "INSERT INTO chatlog (name, message, sent_by, date_created) VALUES(?, ?, ?, ?)";
            $stmt = $db->prepare($query);
            $stmt->bind_param('ssi', $name, $message, $session_id, $currentTime); 
            $stmt->execute(); 
